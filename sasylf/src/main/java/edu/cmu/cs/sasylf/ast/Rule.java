...
public class Rule extends RuleLike implements CanBeCase {
    ...
    public Set<Pair<Term, Substitution>> caseAnalyze(Context ctx, Term term, Element target, Node source) {
        ...
        if (isAssumption()) {
            ...
            tryInsert:
            for (int i = abs.size() - goalAbs.size(); i >= 0; --i) {
    ...
    public edu.cmu.cs.sasylf.types.Rule toTypePb() {
        var builder = edu.cmu.cs.sasylf.types.Rule.newBuilder();

        for (Element premise : this.getPremises()) {
            var sw = new StringWriter();
            var pw = new PrintWriter(sw);
            premise.prettyPrint(pw);
            builder.addPremises(sw.toString());
        }

        var sw = new StringWriter();
        var pw = new PrintWriter(sw);
        this.getConclusion().prettyPrint(pw);
        builder.setConclusion(sw.toString());
