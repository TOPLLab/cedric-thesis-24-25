...
public class Theorem extends RuleLike {
    ...
    public edu.cmu.cs.sasylf.types.Theorem toTypePb() {
        var builder = edu.cmu.cs.sasylf.types.Theorem.newBuilder();

        for (Fact forall : this.getForalls()) {
            builder.addForall(forall.toTypePb());
        }

        var sw = new StringWriter();
        var pw = new PrintWriter(sw);
        this.getExists().prettyPrint(pw);
        builder.setExists(sw.toString());
