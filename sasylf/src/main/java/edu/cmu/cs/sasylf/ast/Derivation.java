...
import edu.cmu.cs.sasylf.types.DerivationFact;
...
public abstract class Derivation extends Fact {
    ...
    public static boolean checkMatchWithImplicitCoercions(Node errorPoint, Context ctx, Element match, Element supplied, String info) {
        ...
        if (match instanceof AndClauseUse) {
            ...
            for (int i = 0; i < matchList.size(); ++i) {
                ...
                if (!checkMatchWithImplicitCoercions(errorPoint, ctx, matchList.get(i), suppliedList.get(i), newMsg))
                    return false;
            }
            return true;
    ...
    public edu.cmu.cs.sasylf.types.Fact toTypePb() {
        var builder = DerivationFact.newBuilder();
